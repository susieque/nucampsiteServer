//This router handles the GET PUT POST and DELETE endpoints. for any path that begins with /campsites, including those using route parameters to spec a particular campsite by its id.
...
campsiteRouter
	...
	.post(cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmin,(req, res, next) => {
		Campsite.findById(req.params.campsiteId)
			.then((campsite) => {
				if (campsite) {
					req.body.author = req.user._id; //it will have the id of user that submitted comment
			...
			if (campsite && campsite.comments.id(req.params.commentId)) {
			  if (
				...
				campsite
				  ...
				  res.statusCode = 403;
				  res.end ("The put operation is not supported for this user");
			  }
			} else if (!campsite) {
			  err = new Error(`Campsite ${req.params.campsiteId} not found`);
			  err.status = 404;
			...
			.then((campsite) => {
				if (campsite && campsite.comments.id(req.params.commentId)) {
					...
					campsite
						...
						res.statusCode = 403;
						res.end('Put operation not supported for this user');
					}
				} else if (!campsite) {
					err = new Error(`Campsite ${req.params.campsiteId} not found`);
					err.status = 404;
